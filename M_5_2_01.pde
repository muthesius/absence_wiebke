...
// wenn ich o klicke, öffne ich den jeweiligen Ordner
void keyReleased() {
  if (key == 'o' || key == 'O') {
...
void keyReleased2(String theFolderPath) {
  if (key == 'p' || key == 'P') {
    saveStrings(theFolderPath, "text.txt");
    //wie kriege ich es richtig hin, dass nach klicken P eine Textdatei gespeichert wird von all den Dateiformaten?
    // Wie sage ich ihm genau, dass er die analysierten Dateien als Textdatei speichern soll..
    // Habe Sorge, dass er jedesmal irgendwo etwas abspeichert..
    
    
      println("\n"+theFolderPath);
      //println zeigt mir unten die Informationen an
  FileSystemItem selectedFolder = new FileSystemItem(new File(theFolderPath)); 
  selectedFolder.printDepthFirst();
  println("\n");
  selectedFolder.printBreadthFirst();
